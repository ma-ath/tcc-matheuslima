...
def create_dataset(X, Y, nof, time_steps=10, target_size=None):
    ...
    for i in range(len(nof)):           # For each video in training_images . . .
        ...
        for j in range(start_index, end_index):     # For each window in this video . . .
            ...
            X_out.append(np.reshape(X[indices], (time_steps, 10, 64, 2)))
...
X1_tr = np.load("fasterRCNN_analysis/M2U00001.json.sparse.npy")
X2_tr = np.load("fasterRCNN_analysis/M2U00002.json.sparse.npy")
X3_tr = np.load("fasterRCNN_analysis/M2U00003.json.sparse.npy")
X4_tr = np.load("fasterRCNN_analysis/M2U00005.json.sparse.npy")
X5_tr = np.load("fasterRCNN_analysis/M2U00006.json.sparse.npy")
...
X1_te = np.load("fasterRCNN_analysis/M2U00004.json.sparse.npy")
X2_te = np.load("fasterRCNN_analysis/M2U00007.json.sparse.npy")
X3_te = np.load("fasterRCNN_analysis/M2U00008.json.sparse.npy")
X4_te = np.load("fasterRCNN_analysis/M2U00012.json.sparse.npy")
X5_te = np.load("fasterRCNN_analysis/M2U00014.json.sparse.npy")
...
#np.save('fasterRCNN_analysis'+'/nof-test.npy', nof_te)
#np.save('fasterRCNN_analysis'+'/nof-train.npy', nof_tr)
